...
| [`agente/`](agente/00_LEIA-ME_INSTRUCOES_PARA_AGENTES.md) | Agentes de IA | Pacote markdown autossuficiente (~48MB) para injetar no contexto de qualquer agente que precise gerar ou avaliar peças de marca VML. Inclui os assets físicos mínimos (logo, cor, tipografia, exemplos) e uma galeria de 44 peças reais aplicadas, em `agente/assets/`. Para colar de uma vez em chats sem upload de arquivo, use [`agente/_pacote_completo.md`](agente/_pacote_completo.md) — os 15 arquivos concatenados em um só (~87KB, só texto, sem os assets binários). |
...
**2026-07-22 (3)** — Descoberta uma lacuna grande: o guideline oficial tem 123 páginas, mas a extração original tinha parado na página 77 — as 44 páginas seguintes (seção "10 Gallery" + páginas finais, quase um terço do documento) nunca tinham sido abertas. Renderizadas e catalogadas pela primeira vez em `agente/15_galeria_de_exemplos.md` + `docs/galeria.html`: 44 peças reais aplicadas (ambientes/sinalização, papelaria, merchandise, digital/social, assinatura de e-mail, Human First) — a única seção do pacote com precedente visual completo em vez de regra abstrata. `asset_manifest.json`, `00_LEIA-ME`, `11_templates_apresentacao.md` e `06_elementos_de_design.md` atualizados para referenciar a nova seção.

**2026-07-22 (4)** — Publicados links públicos permanentes dos 4 arquivos de logo (wordmark preto/branco, ícone preto/branco) via GitHub Pages, para uso direto em `<img src="...">` de qualquer HTML externo: ver `docs/img/logo/`.
